...
class Agent {
    ...
    void computeNewVelocity() {
        ...
        for (final Pair<Double, Obstacle> obstacleNeighbor : obstacleNeighbors) {
            ...
            for (final Line orcaLine : lines) {
                if (RVOMath.det(
                        relativePosition1
                                .scalarMultiply(invTimeHorizonObstacle)
                                .subtract(orcaLine.point),
                        orcaLine.direction)
                        - invTimeHorizonObstacle * radius
                        >= -RVOMath.EPSILON
                        && RVOMath.det(
                        relativePosition2
                                .scalarMultiply(invTimeHorizonObstacle)
            ...
            if (s < 0.0 && distanceSq1 <= radiusSq) {
                ...
                if (obstacle1.convex) {
                    final Vector2D direction =
                            new Vector2D(-relativePosition1.getY(), relativePosition1.getX()).normalize();
            ...
            if (s > 1.0 && distanceSq2 <= radiusSq) {
                ...
                if (obstacle2.convex && RVOMath.det(relativePosition2, obstacle2.direction) >= 0.0) {
                    final Vector2D direction =
                            new Vector2D(-relativePosition2.getY(), relativePosition2.getX()).normalize();
                    lines.add(new Line(Vector2D.ZERO, direction));
                }
            ...
            if (obstacle1.convex
                    && RVOMath.det(leftLegDirection, obstacle1.previous.direction.negate()) >= 0.0) {
            ...
            final Vector2D leftCutOff =
                    obstacle1.point.subtract(position).scalarMultiply(invTimeHorizonObstacle);
            final Vector2D rightCutOff =
                    obstacle2.point.subtract(position).scalarMultiply(invTimeHorizonObstacle);
            ...
            if (distanceSqCutOff <= distanceSqLeft && distanceSqCutOff <= distanceSqRight) {
                ...
                final Vector2D point =
                        leftCutOff.add(
                                radius * invTimeHorizonObstacle, new Vector2D(-direction.getY(), direction.getX()));
                lines.add(new Line(point, direction));
    ...
    void insertObstacleNeighbor(Obstacle obstacle, double rangeSq) {
        ...
        final double r =
                position.subtract(obstacle.point).dotProduct(nextObstacle.point.subtract(obstacle.point))
        ...
        if (r < 0.0) {
            ...
            distSq =
                    position.distanceSq(
                            obstacle.point.add(nextObstacle.point.subtract(obstacle.point).scalarMultiply(r)));
        }
    ...
    /**
     * Solves a one-dimensional linear program on a specified line subject to linear constraints
     ...
     */
    private boolean linearProgram1(
            List<Line> lines, int lineNo, Vector2D optimizationVelocity, boolean optimizeDirection) {
        final double dotProduct = lines.get(lineNo).point.dotProduct(lines.get(lineNo).direction);
        final double discriminant =
                dotProduct * dotProduct + maxSpeed * maxSpeed - lines.get(lineNo).point.getNormSq();
        ...
        for (int i = 0; i < lineNo; i++) {
            ...
            final double numerator =
                    RVOMath.det(lines.get(i).direction, lines.get(lineNo).point.subtract(lines.get(i).point));
    ...
    /**
     * Solves a two-dimensional linear program subject to linear constraints defined by lines and a
     * circular constraint.
     *
     * @param lines Lines defining the linear constraints.
     * @param optimizationVelocity The optimization velocity.
    ...
    private int linearProgram2(
            List<Line> lines, Vector2D optimizationVelocity, boolean optimizeDirection) {
        if (optimizeDirection) {
        ...
        for (int lineNo = 0; lineNo < lines.size(); lineNo++) {
            if (RVOMath.det(lines.get(lineNo).direction, lines.get(lineNo).point.subtract(newVelocity))
                    > 0.0) {
    ...
    private void linearProgram3(int numObstacleLines, int beginLine) {
        ...
        for (int i = beginLine; i < lines.size(); i++) {
            if (RVOMath.det(lines.get(i).direction, lines.get(i).point.subtract(newVelocity))
                    > distance) {
                ...
                for (int j = numObstacleLines; j < i; j++) {
                    ...
                    final Vector2D direction =
                            lines.get(j).direction.subtract(lines.get(i).direction).normalize();

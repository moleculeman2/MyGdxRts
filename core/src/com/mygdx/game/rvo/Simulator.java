...
public class Simulator {
     ...
     *
     * @param position The two-dimensional starting position of this agent.
     * @param neighborDistance The maximum distance (center point to center point) to other agents
     *     this agent takes into account in the navigation. The larger this number, the longer the
     *     running time of the simulation. If the number is too low, the simulation will not be safe.
     *     Must be non-negative.
     * @param maxNeighbors The maximum number of other agents this agent takes into account in the
     *     navigation. The larger this number, the longer the running time of the simulation. If the
     *     number is too low, the simulation will not be safe.
     * @param timeHorizonAgents The minimal amount of time for which this agent's velocities that are
     *     computed by the simulation are safe with respect to other agents. The larger this number,
     *     the sooner this agent will respond to the presence of other agents, but the less freedom
     *     this agent has in choosing its velocities. Must be positive.
     * @param timeHorizonObstacles The minimal amount of time for which this agent's velocities that
     *     are computed by the simulation are safe with respect to obstacles. The larger this number,
     *     the sooner this agent will respond to the presence of obstacles, but the less freedom this
    ...
    /**
     * Adds a new obstacle to the simulation. To add a "negative" obstacle, e.g., a bounding polygon
     * around the environment, the vertices should be listed in clockwise order.
     *
     * @param vertices ArrayList of the vertices of the polygonal obstacle in counterclockwise order.
     * @return The number of the first vertex of the obstacle, or -1 when the number of vertices is
     *     less than two.
     */
    public int addObstacle(List<Vector2D> vertices) {
        ...
        for (int vertexNo = 0; vertexNo < vertices.size(); vertexNo++) {
            ...
            obstacle.direction =
                    vertices
                            .get(vertexNo == vertices.size() - 1 ? 0 : vertexNo + 1)
                            .subtract(vertices.get(vertexNo))
                            .normalize();

            obstacle.convex =
                    vertices.size() == 2
                            || RVOMath.leftOf(
                            vertices.get(vertexNo == 0 ? vertices.size() - 1 : vertexNo - 1),
                            vertices.get(vertexNo),
                            vertices.get(vertexNo == vertices.size() - 1 ? 0 : vertexNo + 1))
                            >= 0.0;
    ...
    /**
     * Performs a simulation step and updates the two-dimensional position and two-dimensional
    ...
    /**
     * Returns the count of agent neighbors taken into account to compute the current velocity for the
     * specified agent.
     *
     * @param agentNo The number of the agent whose count of agent neighbors is to be retrieved.
     * @return The count of agent neighbors taken into account to compute the current velocity for the
     *     specified agent.
     */
    ...
    /**
     * Returns the count of obstacle neighbors taken into account to compute the current velocity for
     * the specified agent.
     *
     * @param agentNo The number of the agent whose count of obstacle neighbors is to be retrieved.
     * @return The count of obstacle neighbors taken into account to compute the current velocity for
     *     the specified agent.
     */
    ...
    /**
     * Returns the ORCA constraints of the specified agent. The half-plane to the left of each line is
     * the region of permissible velocities with respect to that ORCA constraint.
     *
     ...
     *
     * @param agentNo The number of the agent whose two-dimensional preferred velocity is to be
     *     retrieved.
     ...
     *
     * @param agentNo The number of the agent whose time horizon with respect to obstacles is to be
     *     retrieved.
     ...
     *
     * @param agentNo The number of the agent whose two-dimensional linear velocity is to be
     *     retrieved.
    ...
    /**
     * Returns the number of the obstacle vertex succeeding the specified obstacle vertex in its
     * polygon.
     *
     * @param vertexNo The number of the obstacle vertex whose successor is to be retrieved.
     * @return The number of the obstacle vertex succeeding the specified obstacle vertex in its
     *     polygon.
     */
    ...
    /**
     * Returns the number of the obstacle vertex preceding the specified obstacle vertex in its
     * polygon.
     *
     * @param vertexNo The number of the obstacle vertex whose predecessor is to be retrieved.
     * @return The number of the obstacle vertex preceding the specified obstacle vertex in its
     *     polygon.
     */
     ...
     * Obstacles added to the simulation after this function has been called are not accounted for in
     * the simulation.
     */
     ...
     *     in order for the points to be mutually visible (optional). Must be non-negative.
     * @return A boolean specifying whether the two points are mutually visible. Returns true when the
     *     obstacles have not been processed.
     */
     ...
     *
     * @param neighborDistance The default maximum distance (center point to center point) to other
     *     agents a new agent takes into account in the navigation. The larger this number, the longer
     *     he running time of the simulation. If the number is too low, the simulation will not be
     *     safe. Must be non-negative.
     * @param maxNeighbors The default maximum number of other agents a new agent takes into account
     *     in the navigation. The larger this number, the longer the running time of the simulation.
     *     If the number is too low, the simulation will not be safe.
     * @param timeHorizonAgents The default minimal amount of time for which a new agent's velocities
     *     that are computed by the simulation are safe with respect to other agents. The larger this
     *     number, the sooner an agent will respond to the presence of other agents, but the less
     *     freedom the agent has in choosing its velocities. Must be positive.
     * @param timeHorizonObstacles The default minimal amount of time for which a new agent's
     *     velocities that are computed by the simulation are safe with respect to obstacles. The
     *     larger this number, the sooner an agent will respond to the presence of obstacles, but the
     ...
     * @param neighborDistance The replacement maximum neighbor distance. Must be non-negative.
     */
     ...
     * @param agentNo The number of the agent whose two-dimensional preferred velocity is to be
     *     modified.
     * @param preferredVelocity The replacement of the two-dimensional preferred velocity.
     */
     ...
     * @param timeHorizonAgents The replacement time horizon with respect to other agents. Must be
     *     positive.
     */
     ...
     * @param agentNo The number of the agent whose time horizon with respect to obstacles is to be
     *     modified.
     * @param timeHorizonObstacles The replacement time horizon with respect to obstacles. Must be
     *     positive.
     */
